// embed-ort.js — ONNX Runtime Web embeddings for bge-small-en-v1.5
// Replaces wllama embed which crashes on BERT models (llama.cpp WASM b9437 bug).
// Model (~23MB) is downloaded once and cached via Cache API.
...
const CACHE_NAME = 'arkhon-embed-v1';
...
async function loadModel() {
  ...
  ort.env.wasm.wasmPaths  = { 'ort-wasm-simd-threaded.wasm': chrome.runtime.getURL('lib/ort-wasm-simd-threaded.wasm') };

  // Cache model in browser Cache API
  const cache = await caches.open(CACHE_NAME);
  let resp    = await cache.match(MODEL_URL);
  if (!resp) {
    console.log('[arkhon-embed] downloading bge-small-en-v1.5 ONNX (~23MB)…');
...
export async function embed(text) {
  ...
  // bge outputs pooler_output (CLS pooled) or last_hidden_state — prefer pooler_output
  const pooled = out.pooler_output
    ? out.pooler_output.data
    : out.last_hidden_state.data.slice(0, out.last_hidden_state.dims[2]); // CLS token
