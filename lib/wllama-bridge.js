...
const _subs     = new Set();
// Per-tab page indexes — each tab keeps its own BM25 chunk store permanently in memory.
// No cache eviction, no swap-on-switch. Freed when the tab closes.
const _tabStores = new Map(); // tabId → { chunks: string[], url: string }
...
window.__arkhon = {
  ...
  get tabCount()    { return _tabStores.size; },
  ...
  /** True if this tab has a valid page index loaded. */
  hasIndex(tabId) {
    const s = _tabStores.get(tabId);
    return s != null && s.chunks.length > 0;
  },

  /** Free memory when a tab closes. */
  removeIndex(tabId) {
    _tabStores.delete(tabId);
    console.log(`[arkhon] removeIndex tabId=${tabId}, remaining=${_tabStores.size}`);
  },
    ...
    try {
      ...
      */
      ...
      const loadParams = {
        ...
        n_gpu_layers: 0,  // force CPU — WebGPU backend crashes mid-inference, killing the offscreen doc
        progressCallback({ loaded, total }) {
          _progress = total > 0 ? Math.round(loaded / total * 100) : 0; emit();
        },
      };

      await loadWithCache(_chat, CHAT_URL, loadParams);

      _status = 'ready'; _progress = 100; emit();
    } catch (err) {
  ...
  /** Index a page for a specific tab. onProgress({ status, progress }) called per chunk. */
  async indexPage(text, url = '', tabId, onProgress) {
    if (!_chat) throw new Error('Models not loaded');
    const chunks = chunkText(text);
    if (chunks.length === 0) throw new Error('No text found on page');

    onProgress?.({ status: 'indexing', progress: 0 });

    // BM25 — no embeddings needed, just store the text chunks per tab
    const store = [];
    for (let i = 0; i < chunks.length; i++) {
      store.push(chunks[i]);
      onProgress?.({ status: 'indexing', progress: Math.round((i + 1) / chunks.length * 100) });
    }

    _tabStores.set(tabId, { chunks: store, url });
    console.log(`[arkhon] indexPage tabId=${tabId} url=${url} chunks=${store.length}`);
    onProgress?.({ status: 'ready', progress: 100 });
  },

  /** Generate 3 suggested questions about the indexed page for a specific tab. */
  async suggestQuestions(tabId) {
    if (!_chat) return [];
    const tabStore = _tabStores.get(tabId);
    if (!tabStore || tabStore.chunks.length === 0) return [];
    const sample = tabStore.chunks.slice(0, 5).join('\n\n');
  ...
  /** Streaming map-reduce summarization for a specific tab. */
  async summarize(tabId, onToken, onProgress) {
    if (!_chat) throw new Error('Models not loaded');
    const tabStore = _tabStores.get(tabId);
    if (!tabStore || tabStore.chunks.length === 0) throw new Error('Page not indexed');

    const chunks = tabStore.chunks;
    const BATCH = 5;
    let firstBatch = true;
  ...
  async chat(messages, tabId, onToken, { customPrompt = null, maxTokens = 400, k = 4 } = {}) {
    if (!_chat) throw new Error('Models not loaded');
    const tabStore = _tabStores.get(tabId);
    if (!tabStore || tabStore.chunks.length === 0) throw new Error('Page not indexed — click the index button first.');

    const lastUser = [...messages].reverse().find(m => m.role === 'user')?.content ?? '';

    // Per-tab BM25 retrieval — always include first 2 chunks (intro) + top-k by score
    const avg = tabStore.chunks.reduce((s, c) => s + c.split(/\s+/).length, 0) / (tabStore.chunks.length || 1);
    const bm25Chunks = tabStore.chunks
      .map(c => ({ text: c, score: bm25Score(lastUser, c, avg) }))
      .sort((a, b) => b.score - a.score)
      .slice(0, k)
      .map(c => c.text);
    const introChunks = tabStore.chunks.slice(0, 2).filter(c => !bm25Chunks.includes(c));
