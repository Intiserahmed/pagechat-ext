...
async function ensureEmbed() {
  ...
  _embedPromise = (async () => {
    ...
    // Attempting GPU here causes RangeError: Invalid typed array length (WASM OOM).
    const params = { embeddings: true, n_ctx: 512, n_gpu_layers: 0 };
...
window.__arkhon = {
  ...
  async embedFilter(goal, domText, topK = 15, onStatus) {
    ...
    // nomic-embed crashes during createEmbedding inference (wllama GGUF compatibility bug).
    // Keyword filter works well for short DOM labels — semantic ranking not needed here.
    onStatus?.('Filtering elements…');
    const kept = keywordFilter(goal, lines, topK);
    const topLabels = kept.slice(0, 3).map(k => k.line.replace(/^\[\d+\]\s*/, '').slice(0, 30)).join(', ');
    console.log(`[arkhon] embedFilter keyword: kept ${kept.length} (top: ${topLabels})`);
