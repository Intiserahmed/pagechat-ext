...
window.__arkhon = {
   ...
   * - Semantically filtered DOM (done in content script)
  ...
  async agentStep(goal, domTree, history = []) {
    ...
    const prompt = `<start_of_turn>user\nYou are a web agent. Goal: ${goal}\n${histLines}\nPage elements:\n${domTree}\n\nRespond with a single JSON action object.<end_of_turn>\n<start_of_turn>model\n{"action":"`;
...
ws     ::= [ \\t\\n\\r]*`;
    ...
    // The prompt is primed with '{"action":"' so prepend it before parsing
    const json = '{"action":"' + raw.trim().replace(/^[{"]+"?action"?:?"?/i, '');
    try {
      return JSON.parse(json);
    } catch {
      // Fallback: reconstruct from raw string
      const full = (raw.startsWith('{') ? raw : '{"action":"' + raw).replace(/'+$/, '') + '}';
      try { return JSON.parse(full); } catch { return { action: 'done', answer: 'Could not parse response: ' + raw }; }
    }
  },
};
