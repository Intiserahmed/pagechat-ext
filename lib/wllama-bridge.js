...
let _embed     = null;
let _chat      = null;
let _store     = [];      // [{ text: string, emb: number[]|null }]
let _status    = 'idle';
let _progress  = 0;
...
function retrieveBM25(query, k = 3) {
...
function retrieveEmbed(queryEmb, k = 3) {
  return [..._store]
...
window.__pagechat = {
  ...
  async loadModels() {
    ...
    try {
      if (_useEmbed) {
        ...
        await _embed.loadModelFromUrl(EMBED_URL, {
          embeddings: true,
          useCache: false,
      ...
      await _chat.loadModelFromUrl(CHAT_URL, {
        n_ctx: 2048,
        useCache: false,
        progressCallback({ loaded, total }) {
          _progress = total > 0 ? Math.round(loaded / total * 100) : 0; emit();
  ...
  async indexPage(text) {
    ...
    if (_useEmbed && _embed) {
      ...
      // BM25 mode — instant, no model inference
      for (let i = 0; i < chunks.length; i++) {
  ...
  async chat(messages, onToken) {
    ...
    if (_useEmbed && _embed) {
      ...
      topChunks = retrieveEmbed(queryEmb, 3);
    } else {
      topChunks = retrieveBM25(lastUser, 3);
    }

    const context = topChunks.join('\n\n---\n\n');
    const sysPrompt = `You are a helpful assistant answering questions about a webpage the user is reading. Answer based only on the context below. Be concise (2-4 sentences).\n\nContext:\n${context}`;
