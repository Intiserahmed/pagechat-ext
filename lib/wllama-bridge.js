...
window.__arkhon = {
  ...
  async embedFilter(goal, domText, topK = 15, onStatus) {
    ...
    if (_embedDead) {
      ...
      console.log(`[arkhon] embedFilter: embed dead, keyword fallback kept ${kept.length}`);
    } else {
      try {
        const embedReady = !!_embed;
        if (!embedReady) {
          console.log('[arkhon] embedFilter: loading embed model…');
          onStatus?.('Loading embed model…');
        }
        const embed = await ensureEmbed();
        if (!embedReady) onStatus?.('Embed model ready');

        console.log(`[arkhon] embedFilter: embedding goal + ${lines.length} elements`);
        onStatus?.(`Embedding ${lines.length} elements…`);

        const goalEmb = await embed.createEmbedding('Represent this sentence for searching relevant passages: ' + goal);
        const scored = [];
        for (const line of lines) {
          const label   = line.replace(/^\[\d+\]\s*/, '').slice(0, 120);
          const emb     = await embed.createEmbedding(label);
        ...
        console.log(`[arkhon] embedFilter: kept ${kept.length} (scores: ${topScores})`);
        onStatus?.(`Ranked top ${kept.length}`);
      } catch (embedErr) {
        _embedDead = true; _embed = null; _embedPromise = null;
        console.warn(`[arkhon] embedFilter: crashed (${embedErr.message}) — keyword fallback (permanent)`);
        onStatus?.('Keyword filter');
        kept = keywordFilter(goal, lines, topK);
        console.log(`[arkhon] embedFilter keyword fallback: kept ${kept.length}`);
      }
    }
  ...
  async agentStep(goal, domTree, history = []) {
    ...
    const historyStr = history.length > 0
      ? `\nAlready done: ${history.join(', ')}.\n`
      : '\n';
    const prompt = `<start_of_turn>user\nYou are a web agent. Goal: ${goal}${historyStr}\nPage elements:\n${domTree}\n\nRespond with exactly one JSON action.<end_of_turn>\n<start_of_turn>model\n{"action":"`;
...
ws     ::= [ \\t\\n\\r]*`;
    ...
    // (c) nested: model outputs a full JSON object as the string value of "action"
    ...
    if (trimmed.startsWith('{')) {
      try {
        ...
        // (c) Nested case: action field is itself a JSON string e.g. {"action":"{"action":"click","index":0}"}
    ...
    // (d) Extract ANY valid {...} blob anywhere in the full raw string — catches nested objects
